...
fade = True
logo_img = ImageTk.PhotoImage(Image.open(script_dir+'/assets/logo.png').resize(size=(140,75)))
search_img = ImageTk.PhotoImage(Image.open(script_dir+"/assets/search_dark.png").resize(size=(20,20)))
add_img = ImageTk.PhotoImage(Image.open(script_dir+"/assets/add_dark.png").resize(size=(20,20)))
close_img = ImageTk.PhotoImage(Image.open(script_dir+"/assets/close_dark.png").resize(size=(20,20)))
...
def messageScreen(msg):
            ...
            pos_right = round(window.winfo_screenwidth() / 2 - w_window / 2)
            pos_down = round(window.winfo_screenheight() / 2 - h_window / 2)
            messagebox_window.geometry('{}x{}+{}+{}'.format(w_window, h_window, pos_right, pos_down))
            messagebox_window['background'] = color['black1']
            messagebox_window.resizable(False, False)
            frame2 = CTkFrame(master=messagebox_window, corner_radius=5, fg_color=color['black2'], width=360, height=160)
            ...
            btn = CTkButton(text="Close", command=close, corner_radius=4, text_color=color['white'],fg_color=color['red2'], hover_color=color['purple1'], text_font=btn_font,bg_color=color['black2'], height=45,master=frame2)
...
def newUser():
    ...
    def create():
        firstTimeScreen()
    btn = CTkButton(text="Exit", command=close, corner_radius=4, text_color=color['white'],fg_color=color['red1'], hover_color=color['yellow'], text_font=btn_font, bg_color=color['black2'],height=45)
...
def loginScreen():
    ...
    def checkPassword():
        ...
        if password:
            ...
            btn = CTkButton(text="Try Again", command=restart, corner_radius=4, bg_color=color['black2'],
            fg_color=color['green'], hover_color=color['yellow'], text_font=btn_font,text_color=color['black2'])
...
def homeScreen():
    ...
    def addEntry():
        ...
        btn2 = CTkButton(text="Cancel", command=cancel, corner_radius=4, text_color=color['white'],fg_color=color['red1'], hover_color=color['purple1'], text_font=btn_font,bg_color=color['black2'], height=45,master=frame2)
    ...
    searchbox.focus()
    btn = CTkButton(text="Add new", command=addEntry, corner_radius=4, text_color=color['black1'],fg_color=color['green'], hover_color=color['yellow'], text_font=('Inter Bold', 14), bg_color=color['black2'],width=95,master=frame,image=add_img)
    ...
    def search():
        ...
        if len(entry) == 0:
            ...
            frame2.place(x=20, y=20)    
            lbl = Label(frame2, text="Website", bg=color['black2'], fg=color['blue1'], font=('Inter Bold', 12))
            lbl.place(x=50, y=25)
            lbl = Label(frame2, text="Username", bg=color['black2'], fg=color['purple2'], font=('Inter Bold', 12))
            lbl.place(x=205, y=25)
            lbl = Label(frame2, text="Password", bg=color['black2'], fg=color['orange2'], font=('Inter Bold', 12))
            lbl.place(x=360, y=25)
            lbl1 = Label(frame2, text=entry[i][1], font=('Inter Bold', 12),bg=color['black2'],fg=color['blue1'])
            lbl1.place(x=50, y=65 + (i * 40))
            lbl1.configure(anchor=CENTER)   
            lbl2 = Label(frame2, text=(decrypt(entry[i][2], encryptionKey)), font=('Inter Bold', 12),bg=color['black2'], fg=color['purple2'])
            lbl2.place(x=205, y=65 + (i * 40))
            lbl2.configure(anchor=CENTER)   
            lbl3 = Label(frame2, text=(decrypt(entry[i][3], encryptionKey)), font=('Inter Bold', 12),bg=color['black2'], fg=color['orange2'])
            lbl3.place(x=360, y=65 + (i * 40))
            lbl3.configure(anchor=CENTER)
            btn = CTkButton(text="Delete", command=partial(removeEntry, entry[i][0]), corner_radius=4,text_color=color['red2'], fg_color=color['black2'], text_font=('Inter Bold', 12),bg_color=color['black2'], width=65, hover=None,master=frame2)
            btn2 = CTkButton(text="Exit", command=exit_view, corner_radius=4,text_color=color['black1'], fg_color=color['orange1'],bg_color=color['black2'], width=65, hover_color=color['blue1'],master=frame2,text_font=('Inter',25))
